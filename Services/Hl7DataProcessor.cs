...
namespace WarmBox_Central_Monitoring_Station.Services
{
    public static class Hl7DataProcessor
    {
        ...
        private static void LogAlarms(BedViewModel bed, List<string> alarmNames, List<string> alarmTypes)
        {
            ...
            for (int i = 0; i < alarmNames.Count; i++)
            {
                ...
                // 根据报警名称确定优先级（复用 BedViewModel 中的 AlarmPriorityMap 逻辑）
                var priority = BedViewModel.GetAlarmPriority(alarmNames[i]);  // 需要在 BedViewModel 中暴露一个静态方法
                ...
                var log = new AlarmLogItem
                {
                    ...
                    DeviceModel = bed.DeviceType,   // 或 DeviceName
                    ParameterName = null,
                    CurrentValue = null
                };
        ...
        public static void ProcessMessage(string clientIp, string hl7Message, BedViewModel bed)
        {
            try
            {
                ...
                // 5. 记录报警日志
                LogAlarms(bed, alarmNames, alarmTypesForLog);
            }
            catch (Exception ex)
            {
